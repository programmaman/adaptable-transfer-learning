...
def run_structural_node2vec_pipeline(
    ...
    Returns:
        model_with_internal_classifier, test_acc
    """
    device = get_device()
    ...
    # --------------------------------------------------------------------------
    # (B) Initialize Model with Internal Classifier
    # --------------------------------------------------------------------------
    # By setting num_classes to the number of unique labels,
    # the model will include an internal classifier.
    from models.structural_gnn import StructuralGNN

    model_with_internal_classifier = StructuralGNN(
        num_nodes=num_nodes,
        ...
        num_classes=labels.unique().numel(),  # Internal classifier integrated
    ...
    model_with_internal_classifier.train_node2vec(
    ...
    # --------------------------------------------------------------------------
    # Phase 2: Pre-train Full Model (Self-Supervised with Internal Classifier)
    # --------------------------------------------------------------------------
    print("\n=== Phase 2: Pre-training the Structural GNN (with supervision) ===")
    criterion = torch.nn.CrossEntropyLoss()

    pretrain_optimizer = torch.optim.Adam(
        model_with_internal_classifier.parameters(),
        lr=0.01, weight_decay=5e-4
    )

    for epoch in range(full_pretrain_epochs):
        model_with_internal_classifier.train()
        pretrain_optimizer.zero_grad()

        # forward pass returns embeddings and the combined loss (SSL + classification loss)
        embeddings, total_loss = model_with_internal_classifier.forward_and_loss(
            data,
            neg_sample_size=5,
            do_node_class=True,
        ...
        # (Optional) Compute logits for logging purposes:
        logits = model_with_internal_classifier.classify_nodes(embeddings)
        ...
        if epoch % 10 == 0:
            print(
                f"[Pretrain Epoch {epoch:03d}] Total Loss: {total_loss.item():.4f} | (Cls Loss: {cls_loss.item():.4f})"
            )

    def evaluate(model, data_x, edge_index, node_indices, labels, mask, verbose=True):
        model.eval()
        with torch.no_grad():
            logits = model(data_x, edge_index, node_indices)
    ...
    # --------------------------------------------------------------------------
    # Phase 3: Fine-tune for Node Classification (Internal Classifier Only)
    # --------------------------------------------------------------------------
    ...
    for epoch in range(finetune_epochs):
        ...
        # The forward pass now directly returns classification logits.
        logits = model_with_internal_classifier(data_x, edge_index, node_indices)
        loss = criterion(logits[train_mask], labels[train_mask])
        loss.backward()
        finetune_optimizer.step()

        if epoch % 10 == 0:
            model_with_internal_classifier.eval()
            with torch.no_grad():
                print(f"[Fine-tune Epoch {epoch:03d}] Loss: {loss.item():.4f}")
                evaluate(model_with_internal_classifier, data_x, edge_index, node_indices, labels, val_mask)
    ...
    model_with_internal_classifier.eval()
    with torch.no_grad():
        test_logits = model_with_internal_classifier(data_x, edge_index, node_indices)
        test_preds = test_logits[test_mask].argmax(dim=1)
        test_acc = evaluate(model_with_internal_classifier, data_x, edge_index, node_indices, labels, test_mask)
    print("\nFinal Test Evaluation:")

    return model_with_internal_classifier, test_acc
